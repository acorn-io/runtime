...
In the above example the secret renders a template secret with one key call "password.txt" the token from the secret named "token." See [advanced topics](./advanced) for other uses for the template secret type.
