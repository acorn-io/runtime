...
func convert_url_Values_To__LogOptions(in *url.Values, out *LogOptions, s conversion.Scope) error {
	...
	if values, ok := map[string][]string(*in)["includeProxyContainers"]; ok && len(values) > 0 {
		if err := runtime.Convert_Slice_string_To_bool(&values, &out.IncludeProxyContainers, s); err != nil {
			return err
		}
	}
	return nil
